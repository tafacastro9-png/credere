...
if(empty($fecha_corte)){
    echo "<tr><td colspan='4'>Seleccione fecha de corte</td></tr>";
    exit;
}
...
if($tipo == 6){
    ...
    if(empty($mes) || empty($anio)){
        echo "<tr><td colspan='4'>Seleccione mes y año</td></tr>";
        exit;
    }
    ...
    // 🔹 CONSULTAS
    $ingreso_tramite = mysqli_fetch_assoc(mysqli_query($conexion,"
        SELECT SUM(p.monto_prestado * 0.20) total
        FROM prestamos p
        INNER JOIN estado_prestamo ep ON ep.id = p.id_estp
        WHERE MONTH(p.fecha_inicio)='$mes'
        AND YEAR(p.fecha_inicio)='$anio'
    ...
    $ingreso_interes = mysqli_fetch_assoc(mysqli_query($conexion,"
        SELECT SUM(cp.interes) total
        FROM cuotas_prestamo cp
        WHERE cp.estado='PAGADO'
        AND MONTH(cp.fecha_pago)='$mes'
        AND YEAR(cp.fecha_pago)='$anio'
    ...
    $gastos = mysqli_fetch_assoc(mysqli_query($conexion,"
        SELECT SUM(p.monto_prestado * 0.20 * 0.20) total
        FROM prestamos p
        INNER JOIN estado_prestamo ep ON ep.id = p.id_estp
        WHERE MONTH(p.fecha_inicio)='$mes'
        AND YEAR(p.fecha_inicio)='$anio'
    ...
    // 🔥 MAPEO CONTABLE
    $valores = [
        '4105' => $ingreso_tramite,
        '4110' => $ingreso_interes,
        '5105' => $gastos
    ];

    // 🔹 TRAER PLAN DE CUENTAS ORDENADO
    $cuentas = mysqli_query($conexion,"
    ...
    echo "
    <tr>
        <td colspan='4' class='text-center fw-bold' style='background:#1f3b57;color:white;'>
            ESTADO DE RESULTADOS - {$mes}/{$anio}
        </td>
    </tr>

    <tr style='background:#2e59d9;color:white;'>
        <th>CUENTA</th>
        <th>COD</th>
        <th>DB</th>
        <th>CR</th>
    ...
    while($c = mysqli_fetch_assoc($cuentas)){
        ...
        if($c['nivel'] == 1){
            echo "
            <tr>
                <td colspan='4' style='font-weight:bold;color:" . 
                ($c['tipo']=='INGRESO'?'#0d6efd':'#dc3545') . ";
                border-bottom:2px solid ".($c['tipo']=='INGRESO'?'#0d6efd':'#dc3545').";'>
                    {$c['nombre']}
                </td>
            </tr>";
            continue;
        ...
        // 🎯 DB / CR AUTOMÁTICO
        $db = $c['naturaleza'] == 'DB' ? "$ ".f($valor) : "-";
        $cr = $c['naturaleza'] == 'CR' ? "$ ".f($valor) : "-";

        echo "
        <tr>
            <td style='padding-left:{$padding}px'>{$c['nombre']}</td>
            <td>{$c['codigo']}</td>
            <td class='text-end'>{$db}</td>
    ...
    <tr style='border-top:2px solid #000;font-weight:bold;'>
        ...
        <td></td>
        <td>-</td>
        <td>$ ".f($total_ingresos)."</td>
    </tr>

    <tr style='border-top:2px solid #000;font-weight:bold;'>
        <td>Total gastos</td>
        <td></td>
        <td>$ ".f($total_gastos)."</td>
        <td>-</td>
    </tr>";
...
if($tipo == 7){
    ...
    $total_activo = $caja + $cartera;

    // ============================
    // 🔴 PASIVOS
    // ============================

    // 🔥 INVERSIONISTAS (LO QUE DEBES)
    $inversionistas = getTotal($conexion, "
        SELECT IFNULL(SUM(
            CASE 
    ...
    // 🔥 CUENTAS POR PAGAR (SI EXISTE)
    $check = mysqli_query($conexion, "SHOW TABLES LIKE 'cuentas_por_pagar'");
    
    if(mysqli_num_rows($check) > 0){
        $pasivos = getTotal($conexion, "
    ...
    $total_pasivo = $pasivos + $inversionistas;
    ...
    echo "
    <tr>
        <td colspan='5' style='background:#1f3b57;color:white;text-align:center;font-weight:bold;'>
    ...
    <tr><td colspan='4' style='color:red;font-weight:bold;'>ACTIVO</td></tr>
    ...
    <tr style='border-top:2px solid #000;font-weight:bold;'>
        <td>TOTAL ACTIVO</td>
        <td></td>
        <td>$ ".f($total_activo)."</td>
        <td>-</td>
    </tr>

    <tr><td colspan='4' style='color:red;font-weight:bold;'>PASIVO</td></tr>
    ...
    <tr style='border-top:2px solid #000;font-weight:bold;'>
        ...
        <td></td>
        <td>-</td>
        <td>$ ".f($total_pasivo)."</td>
    </tr>

    <tr><td colspan='4' style='color:red;font-weight:bold;'>PATRIMONIO</td></tr>
    ...
    $balance = $total_pasivo + $patrimonio;
    ...
    if(round($total_activo) != round($balance)){
        echo "
        <tr style='background:#f8d7da;color:red;font-weight:bold;'>
            <td colspan='4'>⚠️ ERROR CONTABLE: NO CUADRA</td>
        </tr>";
    } else {
        echo "
        <tr style='background:#d4edda;color:green;font-weight:bold;'>
            <td colspan='4'>✔ BALANCE CUADRADO</td>
        </tr>";
    }
}
?>
